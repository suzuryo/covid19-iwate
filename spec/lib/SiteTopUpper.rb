...
def has_site_top_upper(lang:, data:)
  ...
  # 電話相談をどうぞ
  expect(find('.MainPage > div.v-lazy > span > a:nth-child(1).StaticInfo.Link').text).to eq "#{lang_json['SiteTopUpper']['電話相談をどうぞ']}\n#{lang_json['SiteTopUpper']['相談の手順を見る']}"
  expect(find('.MainPage > div.v-lazy > span > a:nth-child(1).StaticInfo.Link > div > button').text).to eq lang_json['SiteTopUpper']['相談の手順を見る'].to_s
  ...
  if lang == :ja
    ...
    expect(URI(find('.MainPage > div.v-lazy > span > a:nth-child(1).StaticInfo.Link')['href']).path).to eq '/flow/'
  else
    # time
    ...
    expect(URI(find('.MainPage > div.v-lazy > span > a:nth-child(1).StaticInfo.Link')['href']).path).to eq "/#{lang}/flow/"
  end
...
def check_alert_items(lang)
  ALERT_ITEMS.each_with_index do |d, index|
    ...
    expect(find(".MainPage > div.v-lazy > span > a.alertItem:nth-child(#{index + 2}).StaticInfo.#{c}").text).to eq a
    expect(find(".MainPage > div.v-lazy > span > a.alertItem:nth-child(#{index + 2}).StaticInfo.#{c}")['href']).to eq b
  end
end
