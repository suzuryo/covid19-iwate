...
def has_telephone_advisory_reports_number_card
  ...
  # 注釈の中身をチェック
  expect(find('#TelephoneAdvisoryReportsNumberCard .NotesExpansionPanel .v-expansion-panel-content ul > li:nth-child(1)').text).to eq '2021年2月4日を最後に岩手県がこのデータの公表を止めたため、このグラフの更新が止まっている'
  expect(find('#TelephoneAdvisoryReportsNumberCard .NotesExpansionPanel .v-expansion-panel-content ul > li:nth-child(2)').text).to eq '「新型コロナウイルス感染症に関する疑問や心配事がある方からの一般相談」を計上'
  expect(find('#TelephoneAdvisoryReportsNumberCard .NotesExpansionPanel .v-expansion-panel-content ul > li:nth-child(3)').text).to eq 'コールセンター・各保健所等への受付件数の合計'
  expect(find('#TelephoneAdvisoryReportsNumberCard .NotesExpansionPanel .v-expansion-panel-content ul > li:nth-child(4)').text).to eq '窓口を開設した2020年1月21日からのデータを表示'
  expect(find('#TelephoneAdvisoryReportsNumberCard .NotesExpansionPanel .v-expansion-panel-content ul > li:nth-child(5)').text).to eq '岩手県の発表漏れによりデータの存在しない日がある'
