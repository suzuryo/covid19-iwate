  ...
  LOCALES.each do |lang, data|
    context "page #{data[:path]}" do
      ...
      it '項目の値' do
        ...
        expect(find('nav.SideNavigation-Menu > ul.MenuList > li.MenuList-Item:nth-child(3) > a > span.MenuList-Title').text).to eq data[:json]['SideNavigation']['a'][5]

        # MenuList 企業の皆様・はたらく皆様へ
        expect(find('nav.SideNavigation-Menu > ul.MenuList > li.MenuList-Item.-border:nth-child(4) > a > span.MenuList-Icon > span.MenuList-MdIcon > svg > path')[:d]).to eq 'M18,15H16V17H18M18,11H16V13H18M20,19H12V17H14V15H12V13H14V11H12V9H20M10,7H8V5H10M10,11H8V9H10M10,15H8V13H10M10,19H8V17H10M6,7H4V5H6M6,11H4V9H6M6,15H4V13H6M6,19H4V17H6M12,7V3H2V21H22V7H12Z'
        expect(find('nav.SideNavigation-Menu > ul.MenuList > li.MenuList-Item.-border:nth-child(4) > a > span.MenuList-Title').text).to eq data[:json]['SideNavigation']['a'][6]

        # MenuList 岩手県新型コロナウイルス感染症関連情報
        expect(find('nav.SideNavigation-Menu > ul.MenuList > li.MenuList-Item:nth-child(5) > a > span.MenuList-Title').text).to eq data[:json]['SideNavigation']['a'][7]
